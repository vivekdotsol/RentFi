...
pub fn handler(ctx: Context<InitializeVault>, params: InitializeVaultParams) -> Result<()> {
    ...
    vault.lease = ctx.accounts.lease.key();
    ...
    // Mint all RST tokens into the vault's own RST account for primary sale
    let lease_key = ctx.accounts.lease.key();
    let vault_seeds: &[&[u8]] = &[b"vault", lease_key.as_ref(), &[vault.bump]];
    ...
    emit!(VaultInitialized {
        vault_pda: vault.key(),
        lease_pda: ctx.accounts.lease.key(),
    ...
    msg!(
        "RentFi: Vault initialised — {} RST minted at {} USDC each",
        params.total_rst_supply,
        params.rst_price_usdc
    );
